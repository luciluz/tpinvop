...
class InstanciaAsignacionCuadrillas:
    def __init__(self):
        ...
        self.dias = 6
        self.turnos = 5
        self.costo_por_conflicto = -500     #Aca definimos los costos por conflicto y repeticion
        self.costo_por_repeticion = -500    #Definimos los valores como negativos (como queremos que aparezcan en FO)
...
def imprimir_z_i_P_k(prob):
    for i, nombre in enumerate(prob.variables.get_names()):
       # if nombre.startswith('z_') or nombre.startswith('P_') or nombre.startswith('p_') or nombre.startswith('f_'):
